...
function extractAnswerKey(blocks: RawBlock[]): {
  ...
  blocks.forEach((b, i) => {
    ...
    for (const m of t.matchAll(
      /(\d{1,3})\s*[.、):：]\s*([A-Ha-h]+|[√✓×✗]|对|错|正确|错误|[TF])/g,
    )) {
    ...
    for (const m of t.matchAll(/(\d{1,3})\s*[-~—]\s*(\d{1,3})\s*[:：]?\s*([A-Ha-h]+)/g)) {
      ...
      if (e - s + 1 === letters.length)
        for (let n = s; n <= e; n++) answerMap.set(n, letters[n - s])
    }
  })
...
async function blockHash(b: string) {
  return sha256(CACHE_VERSION + '�' + b)
}
